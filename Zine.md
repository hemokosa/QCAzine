...
古典計算は、チューリングマシンによってモデル化され、離散的かつ確定的な古典ビットで表現される決定論的な数を操作することで計算を実行してきた。

量子ビットと量子回路によってモデル化される量子計算は、複素ベクトルによって記述できる量子状態と、複素行列による操作（ユニタリー変換）によって計算を実行する。

量子計算は自然計算でもある。自然計算とは、自然現象を情報処理システムとして捉え、それを直接用いて計算を行う考え方である。古典計算モデルが古典物理世界の現象を扱うのに適していたように、量子計算モデルは量子世界の現象を扱うための枠組みを提供している。

## 量子計算

古典コンピュータの古典ビットは0または1のいずれかの値を取り、これらを組み合わせて整数や実数など、私たちが普段使う数を表現し、計算を行っていた。
...
実数は、「経路（Path）」というイメージスキーマを中核として、大小や順序、移動や伸縮といった数直線上の位置や動きだけで、数や演算が身体化されている。
